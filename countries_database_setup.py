...
import requests
from general_database_operations import execute_query
...
DATABASE_FILE_NAME = "database/countries.db"
...
def fetch_and_insert_data():
    ...
    This function takes no arguments and assumes that the `response` object is defined 
    and contains the data to be fetched. It iterates over the countries in the response,
    extracts relevant information, and inserts it into the database using the 
    `insert_countries_data_to_db` function.

    If an error occurs during the loading process, the error message is printed and the 
    loop continues to the next country.
...
# Create the database and tables calling the execute_query function
for query in queries:
    execute_query(DATABASE_FILE_NAME, query)

# Fetch and insert data into the database calling the fetch_and_insert_data function
